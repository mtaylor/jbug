...
In the Gemfile, under gem "rails", "3.2.3"
...
Devise is a rails engine (http://guides.rubyonrails.org/engines.html), don't worry about what engines are right now, we just need to know that we need to some setup that is specific to devise.
...
Let's give it a whirl.  Check out section 2. Running TorqueBox to deployu your app and start up the server.  Then return here.
...
Now time to test it all out.  Reload your rails environment again: reload!.

Now, lets create 2 users and make them friends with one another.
