...
All the libraries your application dependes on are specified in your applicatons Gemfile (found in your applicaitons root directory).  Let's add in devise and a couple of other dependencies such as a javascript runtime, openssl and jdbc adapter sqlite3.
...
To create our user model and migration (migrations are the mapping between a model and the database) we'll use a model generator.  We don't always have to stick to the generators that come with rails.  3rd party libraries can also offer generators.  To see which generators you have avaiable to you.  You can run
...
If you look at the migration that devise created (under db/migrate) you can see that devise created a new table and added some information such as email, password + the fields we specified (name and dob) devise uses this information to perform a range of authentication.  If you're interested in finding out more about what devise can do for your application check out the devise github repository: https://github.com/plataformatec/devise.git

Now that we've added some migrations we'll create the applications databases and migrate.
