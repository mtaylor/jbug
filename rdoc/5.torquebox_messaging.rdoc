...
and we've also added an extra controller and content to our views for creating and viewing our messages.  Deploy the application and check out the changes.
...
Now we have the relevant libraries, we'll start and create a message processor.  A message processor is basically a ruby message driven bean, we'll use this to get messages from the queue and associate them with each user.  Create a new file tweet_processor.rb under the app/processors directory and add in our logic to handle the message:

  # Note we're just printing to stdout here, check the log for messages
  # Once I figure out how to get the rails env available to the message processor 
  # I'll create the messages for each user
...
Now consumer messages is all set up we just need to create them.  Open up the messages controller 'app/controllers/messages_controller.rb and add in some code to send the message:
